    ...
    def _encode_categorical(self, X: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        for col in categorical_cols:
            if fit:
                ...
                X_encoded[col] = self.encoders[col].transform(
                    X_encoded[col].astype(str)
                )
...
class ModelTrainer:
    ...
    def train_all_models(self, use_hyperparameter_tuning: bool = True) -> Dict:
        ...
        for name, config in self.models_config.items():
            ...
            print(f"   R²: {metrics['r2']:.4f} (CV: {metrics['cv_mean']:.4f} ± {metrics['cv_std']:.4f})")
            print(f"   RMSE: {metrics['rmse']:,.2f} ETB")
            print(f"   MAE: {metrics['mae']:,.2f} ETB")
        
        # Find best model
        self.best_model_name = max(
            self.results, key=lambda x: self.results[x]["r2"]
        )
    ...
    def _hyperparameter_tune(self, model, params: Dict, name: str):
        ...
        if len(params) > 3:
            search = RandomizedSearchCV(
                model, params, n_iter=self.n_iter,
                cv=self.cv_folds, scoring="r2",
                random_state=42, n_jobs=-1
            )
        else:
            search = GridSearchCV(
                model, params, cv=self.cv_folds,
                scoring="r2", n_jobs=-1
            )
    ...
    def save_models(self, output_dir: str = "models") -> None:
        ...
        save_model(
            self.best_model,
            f"{output_dir}/best_model_{timestamp}.joblib"
        )
        ...
        for name, model in self.trained_models.items():
            ...
            save_model(
                model,
                f"{output_dir}/{safe_name}_{timestamp}.joblib"
            )
    ...
    def save_results(self, output_dir: str = "outputs") -> None:
        ...
        results_df.to_csv(
            f"{output_dir}/model_comparison_{timestamp}.csv"
        )
...
class ModelVisualizer:
    ...
    @staticmethod
    def plot_feature_importance(importance_df: pd.DataFrame, top_n: int = 10, save_path: Optional[str] = None) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train housing price prediction models"
    )
    parser.add_argument(
        "--data", "-d",
        type=str,
